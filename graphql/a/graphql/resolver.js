...
const resolvers = {
  Query: {
    ...
    event: async (_, { id }) => await Event.findByPk(id, { include: ["creator"] }),
  },
  Mutation: {
  ...
  try {
    ...
    createEvent: async (_, { title, description, date, location, invitedEmails }, { userId }) => {
      if (!userId) throw new Error("Unauthorized");
      return await Event.create({ title, description, date, location, createdBy: userId, invitedEmails });
    },
